...
const withVoting = (
    ...
    return (props: VotingProps) => {
        ...
        const [ballotEntries, setBallotEntries] = useState<
            IBallotEntry[] | null
        >(null);
        ...
        const { data: ballotEntriesData, loading: loadingBallotEntries } =
            useQuery(ballotEntriesQuery, {
                client: votingClient,
            });

        const { data: votesData, loading: loadingVotes } = useQuery(
            votesQuery,
            {
                client: votingClient,
                variables: {
                ...
                if (ballotEntriesData && props.ballotEntriesProps && election) {
                    const ballotEntriesProxy = props.ballotEntriesProps.map(
                        async (ballotEntry, idx) => {
                            ...
                            return {
                                ...mergedBallotEntry.data.createBallotEntry
                                    .ballotEntry,
                                BallotEntry: BallotEntries[idx],
                            };
                        }
                            ...
                            votes: [
                                {
                                    ...
                                    ballot_entry: {
                                        row: ballotEntry?.ballotEntryByRowId
                                            .row,
                                        election_id: election?.id,
                                    },
                                },
